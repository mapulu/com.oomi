...
class ZwaveDevice extends MeshDevice {
    ...
    _composeCustomSaveMessage(oldSettings, newSettings, changedKeysArr) {
        ...
        if (isBattery === true && isOnline === false) saveMessage = Homey.__(i18n.settings.offlineNodeSaveMessage);
    ...
    async configurationSet(options = {}, value) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (isBattery === true && isOnline === false) return resolve();
        });
    }
    ...
    async configurationGet(options = {}) {
        ...
        if (isBattery === true && isOnline === false) return Promise.reject(new Error('cannot_get_parameter_from_battery_node'));
    ...
    _registerCapabilityGet(capabilityId, commandClassId) {
        ...
        if (capabilityGetObj.opts.getOnStart) {
            ...
            if (isBattery === false) this._getCapabilityValue(capabilityId, commandClassId);
        ...
        if (capabilityGetObj.opts.getOnOnline) {
            ...
            if (isBattery === true && isOnline === true) {
        ...
        this.registerCapabilityListener(capabilityId, (value, opts) => {
            return (async () => {
                ...
                try {
                    ...
                    if (isBattery === true && isOnline === false) return Promise.resolve('TRANSMIT_QUEUED');
